        ...
        if ( pluginMessage.getTag().equals( "BungeeCord" ) )
        {
            ...
            if ( subChannel.equals( "PlayerList" ) )
            {
                ...
                } else
                {
                    ...
                    {
                        out.writeUTF( server.getName() );
                        out.writeUTF( server.getPlayers().stream().map( ProxiedPlayer::getName ).collect( Collectors.joining( ", " ) ) );
                    }
                }
            }
        ...
        if ( !bungee.getPluginManager().callEvent( tabCompleteResponseEvent ).isCancelled() )
        {
            ...
            if ( !commands.equals( tabCompleteResponseEvent.getSuggestions() ) )
            {
                ...
                } else
                {
                    ...
                    tabCompleteResponse.setSuggestions( new Suggestions( range, tabCompleteResponseEvent.getSuggestions()
                        .stream().map( text -> new Suggestion( range, text ) ).collect( Collectors.toList() ) ) );
                }
            }
